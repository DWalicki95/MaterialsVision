...
### Produkcja masek instancyjnych (2026-08-25, [WYKONANE])

Maski powstają **raz, przed treningiem**, skryptem `scripts/build_instance_masks.py` (pakiet `data_prep/masks/`), a nie są odtwarzane z eksportu Label Studio przy każdym runie. Wynik to trzeci zamrożony artefakt obok manifestu i splitu: 707 plików `.tif` w `/home/dwalicki/dane/masks/` (**22 MB** po kompresji zlib), plus `instance_masks.csv` z rozliczeniem per obraz i `instance_masks_metadata.json` z regułami, hashami eksportów LS i hashem manifestu. Kolumna `mask_path` manifestu wskazuje dokładnie na te pliki, więc `mask_exists` jest teraz prawdziwe dla wszystkich 707 wierszy.

Uzasadnienie wyboru „dysk" zamiast „pamięć": znika zależność w złą stronę (`data_prep` importuje `materials_vision`, więc kod treningowy nie może sięgać po czytnik Label Studio), znika ~2.5 min startu każdego runu, a maski da się obejrzeć i zdebugować. Przewidywany koszt 1.2 GB okazał się nieaktualny — obrazy etykiet kompresują się do 22 MB.

**Dwie reguły zamrożone przy rasteryzacji** (plan wcześniej ich nie zawierał; reguła nakładania była jawnie otwarta):

1. **Nakładanie się anotacji: wygrywa poligon narysowany później.** Anotatorzy sporadycznie rysują dwa obrysy dzielące pasek pikseli wzdłuż wspólnej ściany, a piksel może nieść tylko jedną instancję. Zmierzono: sporne piksele to **0.0247%** wszystkich pokrytych, więc każda reguła dałaby niemal identyczne maski — liczy się to, że jest ustalona i odtwarzalna. Eksport LS jest zamrożony, więc jego kolejność też.
2. **Spójność: każda instancja redukowana do największego spójnego kawałka.** To samo nadpisywanie potrafi przewęzić por na pół — dotyczy **957 instancji (2.67%)**. Nie jest to nieszkodliwe, bo cele treningowe dekodera AIS powstają per instancja: por w dwóch kawałkach daje dwa baseny transformaty odległościowej i uczy model dzielić por, który w anotacji jest całością. Koszt zmierzono **przed** przyjęciem reguły: **7 721 px² w całym zbiorze (0.0010% powierzchni anotacji)**, mediana straty na naprawionej instancji 0.01%, P90 0.10%. Dwie instancje z 35 846 tracą ponad piątą część powierzchni, żadna nie traci połowy.

Kontrola zgodności: liczba instancji w każdej z 707 masek **zgadza się z kolumną `n_instances` manifestu** (razem 35 846), a 3 poligony znikają całkowicie, nadpisane przez późniejsze — odnotowane w `instance_masks.csv`. Strona treningowa czyta maski przez `materials_vision/data/masks.py`, który weryfikuje zgodność kształtu z obrazem i **gęstość numeracji ID** (`1..n` bez luk); luka rozjechałaby liczbę instancji z `labels.max()`, czyli po cichu przesunęła indeksowanie tablic per instancja.
